...
public class Service2ApplicationTests {
    ...
    public void beforeTestSettingUp() throws Exception {
        ...
        MvcResult result_lngvr =
                mvc.perform(post(CREATE_LNG_VR_POST_URI_TMPLT).contentType(MediaType.APPLICATION_JSON).
                        ...
                        .andExpect(status().isCreated())
                        .andReturn();
        bizProcIdStr = result_lngvr.getResponse().getContentAsString();
    }

    @After
    public void afterTestCompletion() throws Exception {
        mvc.perform(delete(DELETE_LNG_VR_DELETE_URI_TMPLT + bizProcIdStr).
                param("lingVarId", bizProcIdStr)).andDo(print())
                .andExpect(status().isOk());
    ...
    public void testFindLingVarById() throws Exception {
        mvc.perform(get(READ_BY_ID_LNG_VR_GET_URI_TMPLT + bizProcIdStr).accept(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.lingVarId", is(bizProcIdStr)));
    }
