...
//curl -X POST --data {\"lingVarName\":\"Tester5\",\"lingVarTermLowVal\":\"0\",\"lingVarTermMedVal\":\"3\",\"lingVarTermHighVal\":\"6\"} http://localhost:8191/ling_var_dict/create-ling_var --header "Content-Type:application/json"
    ...
    public ResponseEntity createLingVar(@RequestBody LingVarInfo lingVarInfo) {
        try {
    ...
    public ResponseEntity<List<LingVarInfo>> findAllLingVars() {
        try {
            ...
            if (lingVarsInfo.isEmpty()) {
                return new ResponseEntity(HttpStatus.NOT_FOUND);
            } else {
                return new ResponseEntity<List<LingVarInfo>>(lingVarsInfo, HttpStatus.OK);
            }
        } catch (Exception e) {
    ...
    @ResponseBody
    public ResponseEntity<List<LingVarInfo>> findAllLingVarPaginated(@RequestParam(value = "page", defaultValue = "0", required = true) int page,
                                                                     @RequestParam(value = "sizeLimit", defaultValue = "100", required = true) int sizeLimit) {
        try {
    ...
    public ResponseEntity updateLingVar(@RequestBody LingVarInfo lingVarInfo) {
        try {
    ...
    public ResponseEntity deleteLingVarById(@PathVariable Integer lingVarId) {
        try {
